 ...
 */
package com.weeln.modules.act.web;
...
import com.weeln.modules.act.service.ActTaskService;
...
public class ActTaskController extends BaseController {
	 ...
	 * @return
	 */
	@RequestMapping(value = {"mini/todo", ""})
	public String todoMiniList(Act act, HttpServletResponse response, Model model) throws Exception {
		List<Act> list = actTaskService.todoList(act);
		model.addAttribute("list", list);
		if (UserUtils.getPrincipal().isMobileLogin()){
			return renderString(response, list);
		}
		return "modules/act/actTaskMiniTodoList";
	}
	...
	public String applyList(Act act, HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		Page<Act> page = actTaskService.applyList(new Page<Act>(request, response), act);
		model.addAttribute("page", page);
		if (UserUtils.getPrincipal().isMobileLogin()){
			return renderString(response, page);
		}
		return "modules/act/actTaskApplyList";
	}
...
//		return "modules/act/actTaskForm";
	 ...
	 * 		vars.keys=flag,pass
	 * 		vars.values=1,true
	 * 		vars.types=S,B  @see com.weeln.modules.act.utils.PropertyType
	 */
